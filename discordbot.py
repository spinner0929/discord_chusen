...
async def on_message(message):
    ...
    # 「list」と書き込んだらエントリー者のリストを出力
        ...
        msg = '参加者一覧（現在 ' + str(max(len(list(set(list_entry))), len(list_derby))) + ' 人）：' + str(max(list(set(list_entry)), list_derby, key = len))
    ...
    elif(message.content == 'ダービーパスタ' and message.author.name == 'フクナガ'):
    	list_derby = {}
    ...
    # 「〆切」と書き込むと参加を〆切
    elif(message.content == '〆切' and message.author.name == 'フクナガ'):
        msg = '参加が〆切になりました！'
        flag = 0
    # うめださんが結果順位を書き込むと、全的中者と３連単的中者を表示
    elif(str(message.content).isdecimal() and message.author.name == 'フクナガ' and flag == 0):
    	if len(list_derby) == 0:
            msg = '参加者がいません！'
    	else:
